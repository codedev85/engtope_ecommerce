...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">Hot Deals</h1>
        <h1 class="pull-right">
        ...
        @foreach($products as $product)
            ...
            <td>{!! $product->subcategory['name'] !!}</td>
                ...
                <td>
                    ...
                    {!! Form::close() !!} --}}
                    @if($product->hotdeals == 0)
                    <a class="btn btn-default" href="{{url('/activate/'.$product->id) }}}">Activate</a>
                    @else
                    <a class="btn btn-danger" href="{{ url('/deactivate/'.$product->id) }}">De-Activate</a>
                    @endif
                </td>
            </tr>
        @endforeach
